<?php
    ...
    if($searchCategory == "lect_cate_sec"){
        $pagesql = "SELECT COUNT(*) as cnt from lms_lecture where $searchCategory=".$searchValue;
    }else if($searchCategory == "lect_opt"){
        $Varray =  explode(",", $searchValue);
        $pagesql = "SELECT COUNT(*) as cnt from lms_lecture where lect_opt_prem=$Varray[0] and lect_opt_repre=$Varray[1] and lect_opt_begin=$Varray[2]";
    }else{
        $pagesql = "SELECT COUNT(*) as cnt from lms_lecture where $searchCategory like '%".$searchValue."%'";
    }
    ...
    if($searchCategory == "lect_cate_sec"){
        $sql="SELECT * from lms_lecture where $searchCategory=$searchValue order by lect_key desc limit $page_start, $onePage";
    }else if($searchCategory == "lect_opt"){
        $sql = "SELECT * from lms_lecture where lect_opt_prem=$Varray[0] and lect_opt_repre=$Varray[1] and lect_opt_begin=$Varray[2] order by lect_key desc limit $page_start, $onePage";
    }else{
        $sql="SELECT * from lms_lecture where $searchCategory like '%".$searchValue."%' order by lect_key desc limit $page_start, $onePage";
    }
        ...
        <section>
            ...
            <form action="admin_lecture_list_search.php" method="get" id="searchUiForm" class="input-group w-50" onsubmit="return searchValImport()">
                    <select name="search_category" id="searchCategory" class="form-select">
                        <option value="lect_key" <?php if($searchCategory == "lect_key"){echo "selected";} ?>>강의번호</option>
                        <option value="lect_cate_sec" <?php if($searchCategory == "lect_cate_sec"){echo "selected" ;} ?>>카테고리</option>
                        <option value="lect_name" <?php if($searchCategory == "lect_name"){echo "selected";} ?>>강의명</option>
                        <option value="lect_price" <?php if($searchCategory == "lect_price"){echo "selected";} ?>>수강료</option>
                        <option value="lect_regdate" <?php if($searchCategory == "lect_regdate"){echo "selected";} ?>>등록일</option>
                        <option value="lect_expdate" <?php if($searchCategory == "lect_expdate"){echo "selected";} ?>>만료일</option>
                        <option value="lect_status" <?php if($searchCategory == "lect_status"){echo "selected";} ?>>상태</option>
                        <option value="lect_opt" <?php if($searchCategory == "lect_opt"){echo "selected";} ?>>노출옵션</option>
                    </select>
                    <div id="searchUi" class="w-75">
                        <input type="text" class="form-control search_val" value="<?php echo $searchValue; ?>">
                    </div>
                    <input type="hidden" class="search_content" name="search_val" value="<?php echo $searchValue; ?>">
            ...
            <table class="table table-hover __shadow mt-3">
                ...
                <tr>
                    ...
                    ?></td>
                    <td class="td_Zindex"><a href="admin_lecture_detail.php?page=<?php echo $p->lect_key ?>"><?php echo $p->lect_name ?></td>
                    ...
                    <td>
                        <div class="d-flex justify-content-evenly">
                            ...
                            <div class="form-check">
                                <input id="opt_repre" name="opt_repre" type="checkbox"  <?php if($p->lect_opt_repre == 1){echo "checked";}; ?>  disabled class="opt_repre form-check-input">
                                대표강의
                            </div>
                            <div class="form-check">
                                <input id="opt_begin" name="opt_begin" type="checkbox" <?php if($p->lect_opt_begin == 1){echo "checked";}; ?> disabled class="opt_begin form-check-input">
                                초보추천
                            </div>
                        </div>
                    </td>
                ...
                <?php }
                    ...
                    <tr><td colspan=11>결과가 없습니다</td></tr>
                <?php } ?>
                </tbody>
            </table>
            <ul class="pagination justify-content-center mt-5">
                <li class="page-item <?php if($current_block < 2) echo "disabled"; ?>">
                    <a class="page-link" href="?search_category=<?php echo $searchCategory;?>&search_val=<?php echo $searchValue; ?>&page=<?php echo $current_page-10; ?>" aria-label="Previous 10 pages">
                        <span aria-hidden="true"><i class="fa-solid fa-angles-left"></i></span>
                    </a>
                </li>
                <li class="page-item <?php if($current_page < 2) echo "disabled"; ?>">
                    <a class="page-link" href="?search_category=<?php echo $searchCategory;?>&search_val=<?php echo $searchValue; ?>&page=<?php echo $current_page-1; ?>" aria-label="Previous 1 page">
                ...
                <?php 
                    for($i = $block_start; $i <= $block_end; $i++){
                        if($current_page == $i){
                            echo "<li class='page-item active'><a href='?search_category=$searchCategory&search_val=$searchValue&page=$i' class='page-link'>$i</a></li>";
                        }else{
                            echo "<li class='page-item'><a href='?search_category=$searchCategory&search_val=$searchValue&page=$i'  class='page-link'>$i</a></li>";
                        };
                    };
                ?>

                <li class="page-item <?php if($current_page >= $page_length) echo "disabled"; ?>">
                    <a class="page-link" href="?search_category=<?php echo $searchCategory;?>&search_val=<?php echo $searchValue; ?>&page=<?php echo $current_page+1; ?>" aria-label="Next 1 page">
                        <span aria-hidden="true"><i class="fa-solid fa-angle-right"></i></span>
                    </a>
                </li>
                <li class="page-item <?php if($current_block >= $block_length) echo "disabled"; ?>">
                    <a class="page-link" href="?search_category=<?php echo $searchCategory;?>&search_val=<?php echo $searchValue; ?>&page=<?php echo $current_page+1; ?>" aria-label="Nexts 10 pages">
